...
public class VRCharacterMotor : MonoBehaviour
{
	...
	public static AudioSource SoundSource_Walk;
    ...
    void Awake()
    {
        ...
        tr = transform;
		SoundSource_Walk = (AudioSource) GameObject.Find("WalkSound").GetComponent("AudioSource");
    }

    private void UpdateFunction()
    {
		animation["Idle"].speed = Input.GetAxis("Move Y");
		animation["Walk"].speed = Input.GetAxis("Move Y");			
		
		if(!SoundSource_Walk)
			SoundSource_Walk = (AudioSource) GameObject.Find("WalkSound").GetComponent("AudioSource");
		float runPower = Vector3.SqrMagnitude(movement.velocity);
		SoundSource_Walk.volume = runPower * 0.1f;

		if(!jumping.jumping && grounded)
		{
			if((movement.velocity.x > 0.05f || movement.velocity.x < -0.05f ||
			   movement.velocity.z > 0.05f || movement.velocity.z < -0.05f))
			{
				animation.Play("Walk");
				if(!SoundSource_Walk.isPlaying)
					SoundSource_Walk.Play();
			}
			else
			{				
				animation.Play("Idle");
				SoundSource_Walk.Stop();
			}
		}	
		if(!grounded)		
			SoundSource_Walk.Stop();
        ...
        {			
			audio.PlayOneShot(Sound_Land, -movement.velocity.y*0.05f);
            grounded = true;
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if(grounded)
        {
            ...
            {
				audio.PlayOneShot(Sound_Jump,0.8f);
				animation.Play("Jump");
                grounded = false;
